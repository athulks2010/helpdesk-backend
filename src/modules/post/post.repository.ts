...
export class PostRepository {
  async findAll(query: any = {}) {
    const result: any = { items: [], totalCount: 0, message: 'Posts fetched successfully' }
    try {
      ...
      result.items = []
      result.totalCount = 0
      result.message = err?.message || 'Failed to fetch posts'
      return result
    }
  }

  mapPostPayload(body: any) {
    const payload = { ...body }
    if (payload.content !== undefined && payload.details === undefined) {
      payload.details = payload.content
    }
    if (payload.cover !== undefined && payload.image === undefined) {
      payload.image = payload.cover
    }
  ...
  async create(body: any) {
    try {
      const payload = this.mapPostPayload(body)
  ...
  async restore(id: number | string) {
    try {
      const item = await Post.findByPk(id, { paranoid: false })
      if (!item) throw new Exception({ message: 'Post not found', httpResponseCode: 404 })
      await item.restore()
      return item
    } catch (err: any) {
