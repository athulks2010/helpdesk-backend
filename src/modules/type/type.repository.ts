...
export class TypeRepository {
  async findAll(query: any = {}) {
    const result: any = { items: [], totalCount: 0, message: 'Types fetched successfully' }
    try {
      ...
      const { rows, count } = await Type.findAndCountAll({
        where,
      ...
      result.items = []
      result.totalCount = 0
      result.message = err?.message || 'Failed to fetch types'
      return result
    }
  }
  ...
  async findById(id: number | string) {
    ...
    if (!item) throw new Exception({ message: 'Type not found', httpResponseCode: 404 })
    return item
  }
  ...
  async restore(id: number | string) {
    try {
      const item = await Type.findByPk(id, { paranoid: false })
      if (!item) throw new Exception({ message: 'Type not found', httpResponseCode: 404 })
      await item.restore()
      return item
    } catch (err: any) {
