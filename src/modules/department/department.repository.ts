...
export class DepartmentRepository {
  async findAll(query: any = {}) {
    const result: any = { items: [], totalCount: 0, message: 'Departments fetched successfully' }
    try {
    ...
    if (query?.searchText) {
      ...
      result.items = []
      result.totalCount = 0
      result.message = err?.message || 'Failed to fetch departments'
      return result
    }
  }
  ...
  async findById(id: number | string) {
    ...
    if (!item) throw new Exception({ message: 'Department not found', httpResponseCode: 404 })
    return item
  }
  ...
  async restore(id: number | string) {
    try {
      const item = await Department.findByPk(id, { paranoid: false })
      if (!item) throw new Exception({ message: 'Department not found', httpResponseCode: 404 })
      await item.restore()
      return item
    } catch (err: any) {
